...
Le module `PackFun.js` ajoute deux menus pratiques : `.menuanime` pour les jeux et `.rpgmenu` (alias `.menurpg`) pour toutes les mécaniques RPG. Les activités anime incluent `.persodujour`, `.monpersonnage`, `.combat`, `.fusion`, `.quisuisje`, `.citationanime`, `.opening`, `.blindtest`, `.devineimage`, `.devine`, `.indice`, `.tournoi`, `.vote`, `.tierlist`, `.tier`, `.roulette`, `.pileface`, `.de`, `.defi`, `.actionverite`, `.histoire`, `.aventure`, `.choix`, `.bingoanime`, `.awards`, `.journalgroupe`, `.meteoanime`, `.horoscopeanime`, `.prediction`, `.rivalite`, `.memeanime`, `.eventanime`, `.concoursphoto`, `.scorefun` et `.stopfun`.

Le RPG propose désormais une fiche de héros avec niveau, XP, PV, classe, or et victoires : `.rpg`, `.rpgprofil`, `.rpgstats` et `.rpgrank`. Les joueurs peuvent récupérer une récompense avec `.rpgdaily`, accomplir des missions avec `.rpg mission`, chasser des monstres avec `.rpgchasse`, se soigner avec `.rpgsoin`, gérer leur sac avec `.rpginventaire`, acheter des objets avec `.rpgshop`, et défier un membre avec `.rpgduel @membre`. Le groupe peut également lancer un raid avec `.rpgboss`, puis attaquer avec `.rpgboss hit`. Les données RPG restent en mémoire et sont réinitialisées au redémarrage du bot.
